...
// 
// Solution:
// For a list of numbers [n1, n2, ...] which appear respectively [a1, a2, ...]
// times, the number of different combinations = A(a1 + a2 + ...) / (A(a1) *
// A(a2) * ...). For example, for two 1's and one 2, combinations = A(3) / (
// A(1) * A(2)) = 3. Using a 2-dimensional array to store all solutions (
// appearence times) and calculate the number of combinations for each solution.
// - Sort array first to exclude all numbers greater than target.
// - Cache intermediate results where no solution can be found.
...
int combinationSum4(int* nums, int numsSize, int target)
{
    ...
    // Cache where no solution can be found for a target and starting number
